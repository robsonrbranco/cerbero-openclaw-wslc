...
| `gemini_36_flash.patch.json5` | Registra `gemini-3.6-flash` (1M ctx, $1,50/$7,50 por 1M tok, specs conferidas via WebSearch) e substitui o fallback #1 (era `gemini-3.5-flash`) por ele. **Atualizado em 11/08/2026**: bump pra `gemini-3.7-flash` nos 4 pontos de referência (lista de modelos do provedor Google, fallback #1, alias no select da UI, `webSearch.model` do plugin Google) — mesmo arquivo, nome do modelo trocado in-place | 24/07/2026 |
| `opus_5.patch.json5` | Registra `claude-opus-5` (128K de saída, `api: "anthropic-messages"`), remove `claude-sonnet-4-6`/`claude-opus-4-8` (redundantes com Sonnet 5/Opus 5, confirmado que nenhum dos dois era usado como `model.primary`/`fallbacks` antes de remover) via `models[key]: null` no objeto de aliases + `--replace-path models.providers.anthropic.models` no array de modelos, e corrige `contextWindow` de todos os modelos Anthropic ativos pra 1M (estava 200K por herança do patch original, subestimado). Testado ponta a ponta via `openclaw agent --model anthropic/claude-opus-5` — resposta real, `contextTokens: 1000000` confirmado | 25/07/2026 |
